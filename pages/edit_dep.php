...
if ($_SERVER['REQUEST_METHOD'] == "POST") {
    if (isset($_POST['dep_name'])) {
        if ($_POST['dep_name'] != "" ) {
            ...
            $sql = "UPDATE dep_table SET dep_name='$dep_name' WHERE id='$id'";
            $res = mysqli_query($conn, $sql);
            if ($res) {
                // $success = "تم تحديث القسم او الدائرة بنجاح";
                header('location: view_dep.php');
            } else {
    ...
    $sql="SELECT * FROM dep_table WHERE id='$id'";
    $res = mysqli_query($conn, $sql);
    if($res){
